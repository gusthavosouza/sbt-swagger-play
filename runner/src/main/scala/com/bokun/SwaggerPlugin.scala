...
object SwaggerPluginHelper extends Loggable {
  ...
  def parseRoutes(routesFile: String, prefix: String, env: Environment): List[PlayRoute] = {
    logger.info(s"Processing route file '$routesFile' with prefix '$prefix'")
    ...
    val routes = parsedRoutes
      ...
      .collect {
        case route: PlayRoute =>
          logger.info(s"Adding route '$route'")
        ...
        case include: PlayInclude =>
          logger.info(s"Processing route include $include")
    ...
    logger.info(s"Finished processing route file '$routesFile'")
